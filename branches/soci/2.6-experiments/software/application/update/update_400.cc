...
extern BYTE _binary_idedos_bin_start;
...
extern BYTE _binary_idedos_bin_end;
...
bool program_flash(bool do_update1, bool do_update2, bool do_roms)
{
    ...
    if(do_roms) {
        if(flash->get_page_size() == 528) { // atmel 45.. we should program the roms in one block!
            ...
            copy_rom(roms, FLASH_ID_IDEDOS,    &min, &max, &_binary_idedos_bin_start,   &_binary_idedos_bin_end   );
    	    ...
    	    flash_buffer(FLASH_ID_IDEDOS,    &_binary_idedos_bin_start,   &_binary_idedos_bin_end,   "", "idedos");
...
int main()
{
    ...
    if(calc_checksum(&_binary_1st_boot_400_bin_start, &_binary_1st_boot_400_bin_end) == CHK_1st_boot_400_bin)
